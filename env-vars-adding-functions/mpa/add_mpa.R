# Add MPA (Marine Protected Area) Column to Chick Dataset
...
# The dataset is split across three shapefiles (_0, _1, _2) due to file size constraints, so this script reads all three and combines them before joining.
...
#   5. Points falling inside a polygon are assigned that MPA's name.
#      Points outside all polygons are assigned "No MPA".
#
# Input  : final_chick_data_pre_env_vars.csv
# Output : final_chick_data_mpa.csv
#   New column: mpa — MPA name if inside a protected area, or "No MPA"
...
# Load and combine all WDPA shapefiles
...
# Add a row index to track original observations through the join
chick_sf$row_id <- 1:n
...
cat(paste0("Rows before dedup: ", nrow(joined),
           " (vs ", n, " original — ",
           nrow(joined) - n, " points fell in multiple overlapping MPAs)\n\n"))
...
# Sort so largest area comes first, then keep first row per observation
...
mpa_counts <- sort(table(chick_out$mpa[chick_out$mpa != "No MPA"]), decreasing = TRUE)
print(head(mpa_counts, 15))
...
cat("\nColumn added:\n")
cat("  mpa : MPA name (WDPA NAME field) or 'No MPA' if outside all protected areas\n")
cat("        Where a point fell in multiple overlapping MPAs, the largest was kept.\n")
